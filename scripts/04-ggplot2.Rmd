...
imdb %>%
  ggplot() +
  geom_point(mapping = aes(x = orcamento, y = receita), colour = "purple") +
...
Em vez de rotacionar os rótulos, podemos inverter os eixos usando `coord_flip()`
...
# Gráfico de barras empilhadas (stack)
...
imdb %>% 
  ...
  filter(!is.na(lucrou)) %>%
  count(diretor, lucrou, sort = TRUE) %>% 
  group_by(diretor) %>%
  mutate(
    total_de_filmes = sum(n),
    p_filmes_que_lucraram = sum(n[lucrou])/sum(n)
  ) %>%
  ungroup() %>%
  filter(!is.na(diretor), total_de_filmes > 12) %>% 
  mutate(
    diretor = fct_reorder(diretor, n)
  ) %>% 
  ggplot(aes(x = diretor, y = n, fill = lucrou)) +
...
```{r}
imdb_atores <- imdb %>% 
  gather(importancia_do_ator, nome_do_ator, ator_1, ator_2, ator_3) %>% 
View(imdb_atores)
```
